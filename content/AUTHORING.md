...
- Numbers keep their `.` `,` `/` ("0.08", "1,000", "1/2") and must be whole numbers in the manual: "15" does not match "150".
- Card and question ids must be unique across all lessons. Use only lowercase letters, digits, `-` and `_` (audio file names come from them, and every audio id must be unique).
