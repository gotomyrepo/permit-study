...
- `measureProp(id, x1, x2, y, text, { reach })` is a prop: a labeled "this far" arrow along y (e.g. "20 feet") with ticks at both ends; `reach: [ya, yb]` adds dashed guides from each end to the things it measures between. Hide it until the distance is said. In lane scenes a 36 px car stands for about 15 feet, so 20 feet is 48 px.
