...
- `withExtras(layout, { props, lanes, zones, lines })` adds extra things to any layout. `planArrow(id, from, to, color)` is a "where this car will go" arrow (it curves like `turnPath` when the headings differ). Hide it with `initialStates: { id: 'hidden' }` and show it by setting its state to `''` in the step where it's needed. List blue's arrow last so it's drawn on top.
- People walking: in crosswalk mode, add `FOURWAY.walkLane(dir)` (a lane with `heading: 'any'`) and `FOURWAY.walkZone(dir)` (for `entersAfter`) with `withExtras`. A `pedestrian` is 16 × 12 px, drawn as an orange person seen from above.
