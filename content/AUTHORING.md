...
- Speed: `signProp(id, kind, x, y, { text, size })` in `layouts.ts` is a road sign as a prop, so a step can `highlight` it (e.g. a `speed` sign with `text: '55'`, size 60, on the grass below a `twoLane()` road, on eastbound blue's right). `fogBank(id, x0, x1, { label, opacity })` is a grey-white fog bank (opacity default 0.85) from x0 to x1, covering road and grass (full height, under the cars), with a soft edge that reaches `FOG_EDGE_PX` (22 px) past x0; have blue finish slowing before that edge. `label: 'FOG'` writes a big word on it, and `highlight` rings it while the fog is named. `speedGauge(id, cx, cy, mph, { max, r, ring })` is blue's speedometer: a dial with a blue ring, unlabeled ticks, a red needle and only `mph` written in it, so no unquoted number appears. `signPair(..., { ids: [a, b] })` makes both signs props for `highlight`. Digits get no word timing, so show a number before the gap where it is said.
...
- Driving in to a spot on a word: `driveInTo(start, to, t1, speed)` from `paths.ts` holds a car at `start` (e.g. off screen at x -60), then drives it straight ahead at a steady `speed` (default `SPEED`) so it reaches `to` exactly at `t1`, e.g. so a slow-down with `changeSpeed` starts on the spoken word. It returns `{ start, track }` (use `start` as the actor's start). It throws if `to` isn't straight ahead facing the same way, or if the car would have to set off before t=0.
- Parking: `curbStreet({ hydrant, corner })` in `layouts.ts` is the `twoLane()` road with a parking lane (`CURB.laneId`, y 190–214, `heading: 'any'`) along the curb (prop `CURB.curbId`, `highlight` rings it) and a sidewalk below. Park a car with an `at` pose at center y `CURB.parkY`: its wheels are then 1 foot from the curb. `hydrant` is the x of a fire hydrant on the sidewalk (prop `CURB.hydrantId`, default x `CURB.hydrantX`) or `false`. `corner` is `'stop'` (default: a cross street on the right, a crosswalk `CURB.crosswalkId` from x `CURB.crosswalkX[0]` and a STOP sign `CURB.signId` at x `CURB.signX`), `'crosswalk'` (no sign) or `'none'`. It takes `LayoutExtras`. Every parked car in a picture must be parked legally by every rule the lesson teaches: leave out the hydrant or corner when it would make a car illegal. `wheelsProp(id, pose)` draws a parked car's four tires sticking out `WHEEL_OUT` px past its sides, for a picture about "your wheels".
- `trafficLightProp(id, x, y, { scale, lamps })` in `layouts.ts` is a traffic light as a prop, like `signProp` (e.g. `scale: 1.6` for one the narration names; `highlight` on `id` rings it). Its lamps are a part `<id>-lamps` that starts lit as `lamps` says (`'red'`, or `'red yellow green'` for "a traffic light" that is no one color; default `''`, all dark). A prop's `parts` (`ExtraProp.parts`: id → starting state) are listed as props by `withExtras`; put `...partStates(props)` in the scene's `initialStates` so they start in that state.
- `hydrant(x, y, r)` in `parts.ts` draws the fire hydrant `curbStreet()` uses (`r` default 9; every size scales with it).
- Parking signs: `SignKind` has `no-parking`, `no-standing` and `no-stopping` (white, wider than tall, NO and the word in big red letters, as regulation signs on manual p.29 may be) and `reserved-parking` (the sign pictured on p.44: white, RESERVED PARKING in black, a white wheelchair symbol on a black square). Cite a picture-only fact about the reserved sign with `figure`.
