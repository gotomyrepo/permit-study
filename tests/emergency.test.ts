...
import { B_STOP_T, emergencyBehind, emergencyMoveOver, M_SLOW_FROM_X } from '../src/scenes/defs/emergency';
...
describe('emergency lesson order of events', () => {
  ...
  test('blue starts slowing for the parked ambulance before it reaches it', () => {
    const amb = emergencyMoveOver.actors.find((a) => a.id === 'amb')!;
    const ambRear = amb.start.x - SIZES.ambulance.length / 2;
    expect(M_SLOW_FROM_X + SIZES.car.length / 2).toBeLessThan(ambRear);
  });
});
