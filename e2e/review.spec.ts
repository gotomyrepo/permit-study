...
const CH4 = YAML.parse(readFileSync('content/reader/ch04.yaml', 'utf8')) as { sections: { paragraphs: { source: { page: number } }[] }[] };
const PARAGRAPHS = CH4.sections.reduce((n, s) => n + s.paragraphs.length, 0);
const FIRST_PAGE = CH4.sections[0].paragraphs[0].source.page;
...
test('Manual reader tab lists every paragraph beside its quote', async ({ page }) => {
  ...
  await expect(page.locator('.reader-pane .row').first().getByRole('link', { name: `Manual page ${FIRST_PAGE}` })).toBeVisible();
