...
- `npm run audio`: regenerate narration (`scripts/list-audio-lines.ts` writes `content/audio-lines.json`, then `scripts/build_audio.py` writes `public/audio/<id>.mp3` + `<id>.json` word timings). Only changed lines are re-synthesized. Voice settings: `content/voice.json`; Python deps: `requirements.txt`
...
- `npm run build`: `check --audio` + typecheck + build to `dist/`. CI (`.github/workflows/deploy.yml`) runs `npm test` and `npm run build`, then deploys on push to `main`. CI does not run e2e, so run it locally
...
- **PWA:** `vite-plugin-pwa` with `skipWaiting: false`, so a new version only takes over after every app window is closed. The manual PDF is not precached.
