...
export const B_STOP_T = last(B_PULL_TRACK).t;
...
const B_EASY_TRACK = driveUntil(last(B_SWING_TRACK), last(B_SWING_TRACK).t, B_STOP_T, { speed: B_AMB_SLOW });
const B_EASY: Pose = last(B_EASY_TRACK);
const B_UP_TRACK = changeSpeed(B_EASY, B_AMB_UP, B_STOP_T, B_AMB_SLOW, SPEED);
const B_AMB_TRACK = [
  ...drive(B_AMB_START, at(B_SWING), 0), ...B_SWING_TRACK, ...B_EASY_TRACK, ...B_UP_TRACK,
  ...
  steps: [
    {
      ...
      expect: [{ type: 'stopsBehind', actor: 'blue', line: B_LINE.id, from: B_STOP_T, to: B_MS }],
    },
    {
...
const O_STOP_T = last(O_PULL_TRACK).t;
...
const M_ON = driveUntil(last(M_LC), last(M_LC).t, M_SLOW);
const M_SLOW_FROM: Pose = last(M_ON);
